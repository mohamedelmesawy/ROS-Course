...
class ControlTurtlesim():
    ...
    def __init__(self, linear_x, angular_z):
        ...
        move_cmd.linear.x = linear_x # Modify this value to change the Turtle's speed
...
if __name__ == '__main__':
    ...
    try:
        ...
        while not is_valid:
            linear_x  = int(input("enter linear.x  value between 2 to 6 : "))
            angular_z = int(input("enter angular.z value between 1 to 3 : "))
            if (2 <= linear_x <= 6) and (1 <= angular_z <= 3):
                is_valid = True

        ControlTurtlesim(linear_x, angular_z)
    except:
